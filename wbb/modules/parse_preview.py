...
async def parse(_, message: Message):
    ...
    has_wpp = False
    if not r:
        return await eor(message, text="Veb səhifə ilə mesaja cavab verin")

    m_ = await eor(message, text="Təhlil edilir♻️...")
    ...
    if not has_wpp:
        return await m_.edit(
            "Cavab verilmiş mesajda veb səhifəyə baxış yoxdur.",
        )

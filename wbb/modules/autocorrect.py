...
async def autocorrect_bot(_, message: Message):
    if not message.reply_to_message:
        return await message.reply_text("Mətn mesajınını cavablayın.")
    ...
    if not data.ok:
        return await message.reply_text("Səhv bir şey oldu.")
    result = data.result
...
async def autocorrect_ubot_toggle(_, message: Message):
    ...
    if len(message.command) != 2:
        return await eor(message, text="Kifayət qədər arqument yoxdur.")
    ...
    if state == "enable":
        ...
        return await eor(
            message, text="Səhv arqument, (ENABLE|DISABLE)."
        )
